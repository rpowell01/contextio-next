 ...
 */
function readWebUISettingsOidcPublicUrl() {
...
export function getOidcPublicUrl() {
...
export function getOidcIssuer() {
    return (process.env.CONTEXTIO_OIDC_ISSUER || process.env.OIDC_ISSUER || null);
}
/** Get the OIDC client ID, preferring CONTEXTIO_OIDC_CLIENT_ID over legacy OIDC_CLIENT_ID. */
export function getOidcClientId() {
    return (process.env.CONTEXTIO_OIDC_CLIENT_ID || process.env.OIDC_CLIENT_ID || null);
}
...
export function getOidcClientSecret() {
    return (process.env.CONTEXTIO_OIDC_CLIENT_SECRET || process.env.OIDC_CLIENT_SECRET || null);
}
/** Get the OIDC session secret, preferring CONTEXTIO_OIDC_SESSION_SECRET over legacy OIDC_SESSION_SECRET. */
export function getOidcSessionSecret() {
    return (process.env.CONTEXTIO_OIDC_SESSION_SECRET || process.env.OIDC_SESSION_SECRET || null);
}
/** Get the OIDC scope, merging CONTEXTIO_OIDC_SCOPE and legacy OIDC_SCOPE. */
export function getOidcScope() {
    // Try CONTEXTIO_OIDC_SCOPE first, then legacy OIDC_SCOPE, then defaults
    const scopeString = process.env.CONTEXTIO_OIDC_SCOPE?.split(/\s+/).filter(Boolean) ||
 ...
 */
export function getOidcCallbackUrl(baseUrl) {
    const publicUrl = baseUrl || getOidcPublicUrl();
    if (!publicUrl) {
        throw new Error("CONTEXTIO_OIDC_PUBLIC_URL must be set for OIDC callback URL construction");
    }
