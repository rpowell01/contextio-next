...
let providerMetadataCache: OidcProviderMetadata | null = null;
 ...
 */
function storeState(state: string, nonce: string): void {
 ...
 */
async function getProviderMetadata(issuer: string): Promise<OidcProviderMetadata> {
  const now = Date.now();
 ...
 */
function setSessionCookie(res: http.ServerResponse, cookie: SessionCookie): void {
 ...
 */
export function validateSession(req: http.IncomingMessage, sessionSecret: string): AuthSession | null {
  const cookies = parseCookies(req.headers.cookie);
  const sessionCookie = cookies[SESSION_COOKIE_NAME];
  if (!sessionCookie) return null;

  try {
    const cookie = JSON.parse(sessionCookie) as SessionCookie;
 ...
 */
export function getSessionId(req: http.IncomingMessage, sessionSecret: string): string | null {
...
async function handleLogin(
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      state,
      nonce,
      // Prompt for login each time (can be made configurable)
      // prompt: "login",
    });
    ...
    // Return HTML page with auto-submit form to avoid CORS issues with Next.js RSC prefetch
    const html = `
<!DOCTYPE html>
<html>
<head>
