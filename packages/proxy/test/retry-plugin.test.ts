...
describe("retry plugin - unit tests", () => {
  ...
  describe("onStreamEnd - streaming retry signaling", () => {
    ...
    it("skips retry when bufferOverflow is true and errorDetected is true", async () => {
      ...
      const pendingRetry = (smallBufferPlugin as any)._internal.getAndConsumePendingStreamRetry(sessionId);
    ...
    it("forward.ts streamBufferChunks retains complete data when bufferOverflow occurs (no data loss)", async () => {
      ...
      const forwardStreamBufferChunks: Buffer[] = [];
      ...
      const retryPluginBuffer = (smallBufferPlugin as any)._internal.getStreamBuffer(sessionId);
