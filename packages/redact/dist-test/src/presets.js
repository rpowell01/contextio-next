 ...
 */
function toRule(id, replacement) {
    const cp = CREDENTIAL_PATTERNS.find((p) => p.id === id);
    if (!cp)
        throw new Error(`Unknown credential pattern id: ${id}`);
    ...
    const rule = { name: id, pattern: new RegExp(src, flags), replacement };
    if (cp.allowlist)
        rule.allowlist = cp.allowlist;
    if (cp.minEntropy !== undefined)
        rule.minEntropy = cp.minEntropy;
    return rule;
}
// ---- Secrets preset ----
...
const SECRETS_RULES = [
    ...
    {
        name: "aws-secret-key",
        pattern: /(?<=(?:aws_secret_access_key|secret_?key|SECRET_?KEY)\s*[=:]\s*["']?)[A-Za-z0-9/+=]{40}(?=["']?\s)/g,
...
const PII_RULES = [
    ...
    {
        name: "phone-us",
        pattern: /(?:\+?1[-.\s]?)?\(?[2-9]\d{2}\)?[-.\s]?[2-9]\d{2}[-.\s]?\d{4}\b/g,
    ...
    {
        name: "phone-eu",
        pattern: /\+(?:31|32|33|34|39|41|43|44|45|46|47|48|49)[\s\-\.]?(?:\d[\s\-\.]?){8,11}\b/g,
        replacement: "[PHONE_REDACTED]",
        context: [
            "phone",
            "call",
    ...
    {
        name: "iban",
        pattern: /\b[A-Z]{2}\d{2}(?:[\s]?[A-Z0-9]){11,26}\b/g,
 ...
 */
export function getAllPlaceholderTokens() {
