 ...
 */
import { createRedactPlugin } from "./index.js";
...
async function buildRedactConfig() {
    const settings = await readWebUISettings();
    // Check if redaction is enabled via env or settings
    const redactEnabled = process.env.CONTEXTIO_ENABLE_REDACT !== "false" || settings.redactPreset !== undefined;
    if (!redactEnabled) {
        return null;
    }
    ...
    const config = {
        preset: (settings.redactPreset || process.env.REDACT_PRESET || "pii"),
...
export default async function createRedactPluginFactory() {
