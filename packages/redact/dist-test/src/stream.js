...
function extractContent(json) {
    ...
    if (json.includes("text_delta")) {
        ...
        if (m)
            return { text: m[1], fullMatch: m[0], prefix: m[0].slice(0, m[0].indexOf(m[1])) };
    }
 ...
 */
export function createStreamRehydrator(map) {
    let lineBuf = ""; // Incomplete line from previous chunk
    let contentBuf = ""; // Accumulated text content (may span SSE events)
    let held = []; // Buffered lines awaiting placeholder resolution
    let outputParts = []; // Completed output lines ready to emit
    /**
