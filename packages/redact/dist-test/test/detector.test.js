...
import { createRuleDetector } from "../dist/ruleDetector.js";
import { createDetectorPipeline, createHybridDetector, createDefaultHybridDetector } from "../dist/detectorPipeline.js";
...
let sharedPresidioDetector = null;
let sharedRuleDetector = null;
let sharedPipeline = null;
let sharedHybridPipeline = null;
async function getSharedPresidioDetector() {
...
async function getSharedRuleDetector() {
...
async function getSharedPipeline() {
    if (!sharedPipeline) {
...
async function getSharedHybridPipeline() {
    if (!sharedHybridPipeline) {
        sharedHybridPipeline = await createDefaultHybridDetector({ rules: PRESETS.pii }, { modelName: "Xenova/bert-base-NER", threshold: 0.5, useNER: true });
    }
...
describe("DetectorPipeline", () => {
    ...
    describe("createDefaultHybridDetector factory", () => {
        ...
        it("works with rules only (no LLM config)", async () => {
            const pipeline = await createDefaultHybridDetector({ rules: PRESETS.pii }, undefined);
            try {
                assert.ok(pipeline.isReady());
                const detectors = pipeline.getDetectors();
