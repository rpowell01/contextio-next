...
async function getSharedIntersectionPipeline(): Promise<DetectorPipeline> {
  if (!sharedIntersectionPipeline) {
    const ruleDetector = await getSharedRuleDetector();
    const presidioDetector = await getSharedPresidioDetector();
    sharedIntersectionPipeline = await createDetectorPipeline({
      detectors: [ruleDetector, presidioDetector],
      mergeStrategy: "intersection",
      priorityOrder: ["rules", "presidio-ts"],
    });
  }
  return sharedIntersectionPipeline;
}

async function getSharedLatencyPipeline(): Promise<DetectorPipeline> {
  if (!sharedLatencyPipeline) {
    const ruleDetector = await getSharedRuleDetector();
    const presidioDetector = await getSharedPresidioDetector();
    sharedLatencyPipeline = await createDetectorPipeline({
      detectors: [ruleDetector, presidioDetector],
      mergeStrategy: "priority",
      priorityOrder: ["rules", "presidio-ts"],
    });
  }
  return sharedLatencyPipeline;
}

let sharedIntersectionPipeline: DetectorPipeline | null = null;
let sharedLatencyPipeline: DetectorPipeline | null = null;
...
after(async () => {
  const shutdowns = [
    ...
    sharedIntersectionPipeline?.shutdown() ?? Promise.resolve(),
    sharedLatencyPipeline?.shutdown() ?? Promise.resolve(),
    sharedHybridPipeline?.shutdown() ?? Promise.resolve(),
  ];
  const results = await Promise.allSettled(shutdowns);
  for (const [index, result] of results.entries()) {
    if (result.status === "rejected") {
      const detectorNames = ["PresidioTsDetector", "RuleDetector", "DetectorPipeline", "IntersectionPipeline", "LatencyPipeline", "HybridDetectorPipeline"];
  ...
  sharedPipeline = null;
  sharedIntersectionPipeline = null;
  sharedLatencyPipeline = null;
...
describe("PresidioTsDetector", () => {
  ...
  describe("threshold filtering", () => {
    it("filters out low-confidence detections with high threshold", async function(this: { skip: (msg: string) => void }) {
      skipIfHeavy.call(this);
      // Use shared detector with runtime config to avoid loading model multiple times
      const detector = await getSharedPresidioDetector();
      const result = await detector.detect("John Doe works at Microsoft", { name: "runtime", threshold: 0.9 });
    ...
    it("includes more detections with low threshold", async function(this: { skip: (msg: string) => void }) {
      skipIfHeavy.call(this);
      const detector = await getSharedPresidioDetector();
      const result = await detector.detect("John Doe works at Microsoft", { name: "runtime", threshold: 0.1 });
  ...
  describe("label filtering", () => {
    it("only detects specified labels when configured", async function(this: { skip: (msg: string) => void }) {
      ...
      const result = await detector.detect(
        "Email john@test.com, call +1-555-123-4567, person John Doe",
        { name: "runtime", labels: ["EMAIL_ADDRESS", "PHONE_NUMBER"], threshold: 0.5 }
      );
    ...
    it("supports label aliases (EMAIL -> EMAIL_ADDRESS)", async function(this: { skip: (msg: string) => void }) {
      ...
      const detector = await getSharedPresidioDetector();
      const result = await detector.detect(
        "Email john@test.com, call +1-555-123-4567, SSN 123-45-6789",
        { name: "runtime", labels: ["EMAIL", "PHONE", "SSN"], threshold: 0.5 }
      );
      const labels = result.spans.map(s => s.label);
    ...
    it("warns on unsupported labels", async function(this: { skip: (msg: string) => void }) {
      ...
      try {
        // Need to create a new detector to test initialization warning
  ...
  describe("detector properties", () => {
    it("has correct name and description", async function(this: { skip: (msg: string) => void }) {
      skipIfHeavy.call(this);
      const detector = await getSharedPresidioDetector();
      assert.equal(detector.name, "presidio-ts");
...
describe("DetectorPipeline", () => {
  describe("merge strategies", () => {
    ...
    it("priority merge prefers higher priority detector on overlap", async function(this: { skip: (msg: string) => void }) {
      skipIfIntegration.call(this);
      // Use shared pipeline with "rules" priority over "presidio-ts"
      const pipeline = await getSharedPipeline();
      ...
      // Verify winning detector is the rules detector (priority respected)
      assert.equal(
        emailSpans[0].detectorName,
        "rules",
        `Winning detector should be "rules" (higher priority), got: ${emailSpans[0].detectorName}`
      );
      // Note: using shared pipeline, no shutdown needed (cleaned up in after hook)
    });

    it("intersection merge only keeps detections found by all detectors", async function(this: { skip: (msg: string) => void }) {
      skipIfIntegration.call(this);
      // Use shared intersection pipeline
      const pipeline = await getSharedIntersectionPipeline();
      const result = await pipeline.detect("Email john@test.com and person John Doe");
  ...
  describe("createHybridDetector factory", () => {
    it("creates hybrid config with rules and Presidio detector", async function(this: { skip: (msg: string) => void }) {
      skipIfIntegration.call(this);
      // Use shared detectors to avoid loading model multiple times
      const ruleDetector = await getSharedRuleDetector();
      const presidioDetector = await getSharedPresidioDetector();
      ...
      // Note: using shared detectors, no shutdown needed (cleaned up in after hook)
    });
  ...
  describe("latency tracking", () => {
    it("reports total latency across all detectors", async function(this: { skip: (msg: string) => void }) {
      skipIfIntegration.call(this);
      // Use shared latency pipeline to avoid loading model multiple times
      const pipeline = await getSharedLatencyPipeline();
      const result = await pipeline.detect("Email john@test.com and person John Doe");
