...
function applyMigration(db, migration) {
    const transaction = db.transaction(() => {
        ...
        db.prepare("INSERT INTO schema_version (version, description) VALUES (?, ?)").run(migration.version, migration.name);
    });
    transaction();
}
/**
...
export function getAppliedMigrations() {
    const db = getDb();
    try {
        return db.prepare("SELECT version, applied_at, description FROM schema_version ORDER BY version").all();
    }
    catch (err) {
        // Table doesn't exist yet (fresh database)
        if (err instanceof Error && err.message.includes("no such table")) {
