 ...
 */
function getBaseUrl(headerName, upstreamKey, headers, upstreams, strictUrlForwarding, providerConfigs, provider) {
    if (strictUrlForwarding) {
        const upstreamValue = upstreams[upstreamKey];
        const headerValue = headers[headerName];
        if (headerValue && headerValue !== upstreamValue) {
            console.warn(`[StrictURLForwarding] Ignoring ${headerName} "${headerValue}", using configured upstream "${upstreamValue}"`);
        }
    ...
    if (headerValue) {
        ...
        if (process.env.DEBUG_ROUTING === "true") {
            console.error(`[DEBUG_ROUTING] Using ${headerName} header: ${normalized}`);
        }
        return normalized;
    }
    return upstreams[upstreamKey];
 ...
 */
export function classifyRequest(pathname, headers, strictUrlForwarding = false, upstreams) {
    if (process.env.DEBUG_ROUTING === "true") {
        console.log(`[DEBUG_ROUTING] classifyRequest: pathname=${pathname}, strictUrlForwarding=${strictUrlForwarding}, upstreams=${upstreams ? 'provided' : 'undefined'}`);
    }
    ...
    if (process.env.DEBUG_ROUTING === "true") {
        console.error(`[DEBUG_ROUTING] NVIDIA check: strictUrlForwarding=${strictUrlForwarding}, x-nvidia-baseurl=${nvidiaBaseUrl || "NOT SET"}`);
    }
    if (!strictUrlForwarding &&
        nvidiaBaseUrl)
            ...
            for (const [upstreamProvider, upstreamUrl] of Object.entries(upstreams)) {
                if (upstreamUrl) {
                    ...
                    if (hostname === upstreamUrlObj.hostname || hostname.endsWith(`.${upstreamUrlObj.hostname}`)) {
                        ...
                        let apiFormat = "chat-completions";
                        if (upstreamProvider === "anthropic")
                            apiFormat = "anthropic-messages";
                        else if (upstreamProvider === "gemini" || upstreamProvider === "vertex")
                            apiFormat = "gemini";
                        else if (upstreamProvider === "chatgpt")
                            apiFormat = "chatgpt-backend";
                        else if (upstreamProvider === "openai")
                            apiFormat = "chat-completions";
                        if (process.env.DEBUG_ROUTING === "true") {
                            console.error(`[DEBUG_ROUTING] Provider fallback detection via x-target-url: ${upstreamProvider} (${hostname})`);
                        }
                        return { provider: upstreamProvider, apiFormat };
                    }
                }
            }
        }
 ...
 */
export function resolveTargetUrl(pathname, search, headers, upstreams, strictUrlForwarding = false, providerConfigs) {
    const { provider, apiFormat } = classifyRequest(pathname, headers, strictUrlForwarding, upstreams);
    const qs = search || "";
    ...
    if (process.env.DEBUG_ROUTING === "true") {
        console.error(`[DEBUG_ROUTING] pathname=${pathname}, provider=${provider}, apiFormat=${apiFormat}`);
        console.error(`[DEBUG_ROUTING] x-target-url=${headers["x-target-url"] || "none"}`);
    }
    if (!targetUrl) {
        if (provider === "chatgpt") {
            ...
            targetUrl =
                getBaseUrl(isCodeAssist ? "x-gemini-code-assist-baseurl" : "x-gemini-baseurl", isCodeAssist ? "geminiCodeAssist" : "gemini", headers, upstreams, strictUrlForwarding, providerConfigs, provider) + pathname + qs;
        }
            ...
            const openaiPath = pathname === "/responses" ? "/v1/responses" : pathname;
    ...
    if (targetUrl) {
        try {
            ...
            const classifiedUpstreamUrl = upstreams[provider];
            ...
            for (const [upstreamProvider, upstreamUrl] of Object.entries(upstreams)) {
                if (upstreamUrl) {
                    ...
                    if (targetUrlObj.hostname === upstreamUrlObj.hostname) {
                        ...
                        const shouldOverride = targetUrlFromHeader ||
                            provider === "unknown" ||
                            !matchesClassifiedUpstream;
                        if (shouldOverride) {
                            finalProvider = upstreamProvider;
                            ...
                            if (process.env.DEBUG_ROUTING === "true") {
                                console.error(`[DEBUG_ROUTING] Second-pass: Overriding provider to ${upstreamProvider} based on target URL hostname: ${targetUrlObj.hostname}`);
                            }
                        }
                        break;
                    }
    ...
    if (process.env.DEBUG_ROUTING === "true") {
        console.error(`[DEBUG_ROUTING] Result: targetUrl=${targetUrl}, provider=${finalProvider}`);
    }
