...
function isRateLimiterPlugin(plugin) {
    return (plugin.name === "rate-limiter" &&
        "_internal" in plugin &&
        typeof plugin._internal?.getAllKeys === "function" &&
        typeof plugin._internal?.shutdown === "function");
}
/**
 ...
 */
async function createTestProxy(upstreamPort, rateLimiterConfig) {
...
describe("rate-limiter integration", () => {
    ...
    after(async () => {
        if (rateLimiterPlugin && isRateLimiterPlugin(rateLimiterPlugin))
            rateLimiterPlugin._internal.shutdown();
        if (bufferingPlugin && isRateLimiterPlugin(bufferingPlugin))
            bufferingPlugin._internal.shutdown();
        if (cleanupPlugin && isRateLimiterPlugin(cleanupPlugin))
            cleanupPlugin._internal.shutdown();
        if (proxy)
            await proxy.stop();
        if (bufferingProxy)
            await bufferingProxy.stop();
        if (cleanupProxy)
            await cleanupProxy.stop();
        if (upstreamServer)
...
//# sourceMappingURL=rate-limiter.integration.test.js.map