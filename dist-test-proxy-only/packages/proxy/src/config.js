...
import { DEFAULT_OIDC_SCOPE, KNOWN_PROVIDERS, validateProviderConfig } from "@contextio/core";
...
function readWebUISettings() {
    ...
    const paths = [localSettingsPath, dockerSettingsPath].filter(Boolean);
 ...
 */
export function readProvidersConfig(filePath = getProvidersFilePath()) {
 ...
 */
function readProvidersConfigFromFile(filePath) {
    try {
...
function getCaptureCleanupMaxAgeMs() {
...
function getCaptureCleanupIntervalMs() {
...
function getCaptureCleanupEnabled(maxAgeMs) {
 ...
 */
export function resolveOidcConfig(overrides) {
...
export function resolveConfig(overrides) {
    ...
    const upstreams = {
        openai: process.env.UPSTREAM_OPENAI_URL ||
            providersConfig.openai?.upstreamUrl ||
            "",
        anthropic: process.env.UPSTREAM_ANTHROPIC_URL ||
            providersConfig.anthropic?.upstreamUrl ||
            "",
        chatgpt: process.env.UPSTREAM_CHATGPT_URL ||
        ...
        geminiCodeAssist: process.env.UPSTREAM_GEMINI_CODE_ASSIST_URL ||
            "",
        vertex: process.env.UPSTREAM_VERTEX_URL ||
            providersConfig.vertex?.upstreamUrl ||
            "",
        nvidia: process.env.UPSTREAM_NVIDIA_URL ||
            providersConfig.nvidia?.upstreamUrl ||
            "",
        kilo: process.env.UPSTREAM_KILO_URL ||
            providersConfig.kilo?.upstreamUrl ||
            "",
        openrouter: process.env.UPSTREAM_OPENROUTER_URL ||
    ...
    const allowTargetOverride = overrides?.allowTargetOverride ??
        process.env.CONTEXT_PROXY_ALLOW_TARGET_OVERRIDE === "1";
    const strictUrlForwarding = overrides?.strictUrlForwarding ??
        process.env.STRICT_URL_FORWARDING === "true";
    const loggerCaptureDir = overrides?.loggerCaptureDir ||
    ...
    const loggerEncryption = {
        enabled: overrides?.loggerEncryption?.enabled ??
    ...
    function parseRateLimitForProvider(provider, fileConfig) {
    ...
    function validateRateLimitConfig(config) {
        if (!Number.isFinite(config.maxRequests) || config.maxRequests < MIN_MAX_REQUESTS || config.maxRequests > MAX_MAX_REQUESTS) {
            throw new Error(`Rate limiter maxRequests must be a finite number between ${MIN_MAX_REQUESTS} and ${MAX_MAX_REQUESTS} (got: ${config.maxRequests})`);
        }
        if (!Number.isFinite(config.windowMs) || config.windowMs < MIN_WINDOW_MS || config.windowMs > MAX_WINDOW_MS) {
            throw new Error(`Rate limiter windowMs must be a finite number between ${MIN_WINDOW_MS} and ${MAX_WINDOW_MS} (got: ${config.windowMs})`);
        }
        if (!Number.isFinite(config.bufferCapacity) || config.bufferCapacity < MIN_BUFFER_CAPACITY || config.bufferCapacity > MAX_BUFFER_CAPACITY) {
            throw new Error(`Rate limiter bufferCapacity must be a finite number between ${MIN_BUFFER_CAPACITY} and ${MAX_BUFFER_CAPACITY} (got: ${config.bufferCapacity})`);
        }
    }
    const rateLimiter = {
    ...
    function resolveRetryForProvider(provider, fileConfig) {
