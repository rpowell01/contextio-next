...
import { extractSource, resolveTargetUrl, selectHeaders, } from "@contextio/core";
 ...
 */
async function runRequestPlugins(plugins, ctx) {
    let current = ctx;
    for (const plugin of plugins) {
        ...
        try {
            ...
            console.error(`Plugin "${plugin.name}" onRequest error:`, err instanceof Error ? err.message : String(err));
        }
    }
    return current;
}
/** Run onResponse hooks as a pipeline (same fail-open semantics as onRequest). */
async function runResponsePlugins(plugins, ctx) {
    let current = ctx;
    for (const plugin of plugins) {
        ...
        try {
            ...
            console.error(`Plugin "${plugin.name}" onResponse error:`, err instanceof Error ? err.message : String(err));
        }
    }
    return current;
}
/** Fire all onCapture hooks. Errors are logged but never block the response. */
function runCapturePlugins(plugins, capture) {
    for (const plugin of plugins) {
        if (!plugin.onCapture)
            continue;
        try {
            ...
            if (result && typeof result.catch === "function") {
                result.catch((err) => {
                    console.error(`Plugin "${plugin.name}" onCapture async error:`, err instanceof Error ? err.message : String(err));
                });
            }
        }
        catch (err) {
            console.error(`Plugin "${plugin.name}" onCapture error:`, err instanceof Error ? err.message : String(err));
        }
    }
}
/**
 ...
 */
function processStreamChunk(chunk, sessionId, plugins, hasStreamPlugins, jsonBuffer, isStreaming) {
        ...
        if (hasStreamPlugins) {
            for (const plugin of plugins) {
                if (!plugin.onStreamChunk)
                    continue;
                try {
                    let ret = plugin.onStreamChunk(buf, sessionId);
                    if (ret instanceof SharedArrayBuffer) {
                        ret = Buffer.from(Buffer.from(ret));
                    }
                    ...
                    console.error(`Plugin "${plugin.name}" onStreamChunk error:`, err instanceof Error ? err.message : String(err));
                }
            }
        }
...
function buildCaptureData(options) {
    return {
        ...
        responseHeaders: selectHeaders(options.proxyRes.headers),
 ...
 */
function reclassifyProviderFromResponse(initialProvider, proxyRes, responseBody, logTraffic) {
    ...
    if (serverHeader.includes("nvidia") ||
    ...
    if (serverHeader.includes("openrouter") ||
    ...
    if (serverHeader.includes("chatgpt") ||
    ...
    if (serverHeader.includes("openai") ||
 ...
 */
function headersForResolution(headers, allowTargetOverride, logTraffic) {
    const h = headers;
    if (logTraffic) {
...
export function createProxyHandler(opts) {
    ...
    return function handleProxy(req, res) {
        ...
        let sessionId = headers["x-kilo-session"] ||
        ...
        const routingHeaders = headersForResolution(req.headers, opts.allowTargetOverride, opts.logTraffic);
        if (opts.logTraffic) {
            console.error(`[DEBUG] Routing headers: ${JSON.stringify(routingHeaders, null, 2)}`);
        }
        const { targetUrl, provider, apiFormat } = resolveTargetUrl(cleanPath, search, routingHeaders, opts.upstreams, opts.strictUrlForwarding, opts.providers);
        if (opts.logTraffic) {
            const hasAuth = !!req.headers.authorization;
            const headerKeys = Object.keys(req.headers);
            console.error(`[DEBUG] Routing: provider=${provider}, apiFormat=${apiFormat}, targetUrl=${targetUrl}, auth=${hasAuth}`);
            console.error(`[DEBUG] Headers: ${headerKeys.map((k) => `${k}=${req.headers[k]}`).join(", ")}`);
        }
        if (!targetUrl) {
            if (opts.logTraffic) {
                console.error(`[DEBUG] Unknown provider for path '${cleanPath}', provider=${provider}`);
            }
        ...
        if (opts.logTraffic) {
            ...
            console.error(`${req.method} ${req.url} → ${targetUrl} [${provider}] ${sourceTag} auth=${hasAuth}`);
        }
        ...
        const captureId = source !== null && source !== undefined
        ...
        req.on("end", () => {
            ...
            const doForward = (ctx) => {
                ...
                const targetParsed = url.parse(targetUrl);
                const forwardHeaders = buildForwardHeaders(ctx.headers, targetParsed.host, forwardBuffer.length);
                const protocol = targetParsed.protocol === "https:" ? https : http;
                ...
                const proxyReq = protocol.request({
                    ...
                    const flushBufferAndStream = (chunkToWrite) => {
                    ...
                    proxyRes.on("data", (chunk) => {
                        ...
                        if (!shouldBufferResponse && !res.destroyed) {
                            ...
                            if (hasStreamPlugins && isStreaming) {
                                ...
                                const result = processStreamChunk(chunk, sessionId, plugins, hasStreamPlugins, jsonBuffer, true);
                    ...
                    proxyRes.on("end", () => {
                        ...
                        if (hasStreamPlugins && isStreaming && !res.destroyed) {
                            for (const plugin of plugins) {
                                ...
                                try {
                                    ...
                                    console.error(`Plugin "${plugin.name}" onStreamEnd error:`, err instanceof Error ? err.message : String(err));
                                }
                            }
                        }
                        ...
                        if (shouldBufferStreamForRetry && !streamBufferOverflow && !res.destroyed) {
                            ...
                            const pendingRetry = retryPlugin &&
                                typeof retryPlugin._internal?.getAndConsumePendingStreamRetry === "function"
                                ? retryPlugin._internal.getAndConsumePendingStreamRetry(sessionId)
                                : null;
                            if (pendingRetry) {
                        ...
                        const finishResponse = (finalBody, finalHeaders, finalStatus) => {
                            ...
                            if (hasCapturePlugins) {
                                ...
                                if (sessionId?.startsWith("title-")) {
                                    ...
                                    const reclassified = reclassifyProviderFromResponse(provider, proxyRes, finalBodyStr, opts.logTraffic);
                                    const captureProvider = reclassified.reclassified
                                        ? reclassified.provider
                                        : provider;
                                    const captureApiFormat = reclassified.reclassified
                                        ? reclassified.apiFormat
                                        : apiFormat;
                                    if (reclassified.reclassified && opts.logTraffic) {
                                        console.error(`[FORWARD] Provider reclassified from ${provider} to ${captureProvider} based on response`);
                                    }
                        ...
                        if (shouldBufferStreamForRetry) {
                            ...
                            runResponsePlugins(plugins, respCtx)
                                ...
                                if (finalCtx.status === 599) {
                                    ...
                                    try {
                                        ...
                                        if (retryId || captureId) {
                                            const retryPlugin = plugins.find((p) => p.name === "retry");
                                            if (retryPlugin && retryPlugin._internal?.getRequestBody) {
                                                ...
                                                let bodyBuffer;
                                                let bodyJson;
                                                if (hasModifiedBodyHeader && retryPlugin._internal?.getModifiedBodyForRetry) {
                                                    // Use modified body with "continue" message for NVIDIA retry
                                                    bodyBuffer = retryPlugin._internal.getModifiedBodyForRetry(retryId || "") ??
                                                        (captureId ? retryPlugin._internal.getModifiedBodyForRetry(captureId) : undefined);
                                                    ...
                                                    bodyBuffer = retryPlugin._internal.getRequestBody(retryId || "") ??
                                                        (captureId ? retryPlugin._internal.getRequestBody(captureId) : undefined);
                                                    bodyJson = retryPlugin._internal.getRequestBodyJson?.(retryId || "") ??
                                                        (captureId ? retryPlugin._internal.getRequestBodyJson?.(captureId) : undefined);
                                                }
                                                if (bodyBuffer) {
                                        ...
                                        finishResponse(JSON.stringify({ error: "Proxy error", service: SERVICE_IDENTIFIER }), { "content-type": "application/json" }, 502);
                                    }
                                    return;
                                }
                                ...
                                .catch((err) => {
                                console.error("Response plugin pipeline error:", err instanceof Error ? err.message : String(err));
                                finishResponse(respBody, proxyRes.headers, proxyRes.statusCode || 0);
                            });
                        }
                        else {
                            finishResponse(respBody, proxyRes.headers, proxyRes.statusCode || 0);
            ...
            function isRateLimitError(err) {
                return err instanceof Error && err.statusCode === 429;
            }
            // Run request plugins, then forward
            let currentCtx = reqCtx;
            if (hasRequestPlugins) {
                runRequestPlugins(plugins, reqCtx)
                    ...
                    console.error("Request plugin pipeline error:", err instanceof Error ? err.message : String(err));
