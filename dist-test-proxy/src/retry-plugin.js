 ...
 */
function resolveConfigForProvider(globalConfig, providerConfigs, provider) {
    if (!providerConfigs) {
        return globalConfig;
    }
...
class RetryPlugin {
    ...
    nvidiaWorkerRetryCount = 0;
    // Counter for upstream 429 responses per provider
    upstream429Counts = new Map();
    constructor(config = {}) {
    ...
    cleanupStaleEntries() {
        ...
        if (this.requestStore.size > this.maxEntries) {
            ...
            for (const key of this.requestStore.keys()) {
                if (keysToRemove.length >= toRemove)
                    break;
    ...
    processSseLine(trimmed, state) {
        ...
        if (state.inDataField && state.dataBuffer.length > 0 && trimmed === "") {
            ...
            if (rateLimitCheck.isRateLimit) {
                const shouldUpdate = !state.errorDetected || state.errorStatus === null;
                if (shouldUpdate) {
                    state.errorDetected = true;
    ...
    parseSseForError(chunk) {
        ...
        if (state.inDataField && state.dataBuffer.length > 0) {
            ...
            // Also check for rate limit indicators in buffered data (only if no numeric status)
            const rateLimitCheck = this.checkRateLimitInData(state.dataBuffer);
            if (rateLimitCheck.isRateLimit) {
     ...
     */
    checkNvidiaResourceExhausted(responseBody) {
        if (!responseBody)
     ...
     */
    registerStreamRetryModifier(provider, modifier) {
     ...
     */
    createRetryBody(originalBodyJson, errorType, provider, originalBodyBuffer) {
        switch (errorType) {
            case 'nvidia':
     ...
     */
    getStorageKey(captureId, requestId) {
     ...
     */
    isRetryableStatus(status, config) {
     ...
     */
    cleanupAllState(streamState, sessionId) {
        this.streamState.delete(sessionId);
        const storageKey = this.getStorageKey(streamState.captureId, streamState.requestId);
        this.requestStore.delete(storageKey);
    }
    ...
    onStreamEnd(sessionId) {
        ...
        if (streamState.errorDetected) {
            ...
            if (entry) {
                ...
                // Fallback: if error detected but no numeric status, check if error message indicates rate limit
                // Handles Anthropic-style errors (type: "error", error: { type: "rate_limit_error", ... })
                // and bare SSE "event: error" events that may not have numeric status codes
                if (!errorType && streamState.errorDetected) {
                    // Collect all available error text for rate limit detection
                    const errorTexts = [];
                    if (streamState.errorMessage)
                        errorTexts.push(streamState.errorMessage);
                    if (streamState.dataBuffer)
                        errorTexts.push(streamState.dataBuffer);
                    for (const errorText of errorTexts) {
                        const errorMsg = errorText.toLowerCase();
                        console.debug(`[retry] Fallback check - errorMsg: "${errorMsg}", hasErrorEvent: ${streamState.hasErrorEvent}`);
                        // Check for common rate limit indicators in error message
                        // Be specific to avoid false positives (e.g., "Error 0" should not match)
                        const isRateLimitError = errorMsg.includes('rate_limit') ||
                            ...
                            (streamState.hasErrorEvent && (errorMsg.includes('rate_limit') ||
                                errorMsg.includes('rate limit') ||
                                errorMsg.includes('ratelimit') ||
                                errorMsg.includes('rate-limit') ||
                                errorMsg.includes('too many requests') ||
                                errorMsg.includes('quota exceeded') ||
                                errorMsg.includes('throttle')));
                        console.debug(`[retry] Fallback isRateLimitError: ${isRateLimitError}`);
                        if (isRateLimitError) {
                            errorType = 'provider-specific';
                            console.debug(`[retry] Provider-specific rate limit error detected (no numeric status): ${errorText}`);
                            break;
                        }
                    }
                }
     ...
     * Can look up by captureId or requestId.
     */
    getRequestBodyJsonForTesting(key) {
        const entry = this.requestStore.get(key);
     ...
     * Can look up by captureId or requestId.
     */
    getRequestHeadersForTesting(key) {
        const entry = this.requestStore.get(key);
     ...
     */
    incrementUpstream429Count(provider) {
     ...
     */
    getStreamErrorForTesting(sessionId) {
     ...
     */
    getAndConsumePendingStreamRetry(sessionId) {
        if (!sessionId)
            return null;
        const streamState = this.streamState.get(sessionId);
     ...
     */
    getStreamBufferForTesting(sessionId) {
        const streamState = this.streamState.get(sessionId);
     ...
     */
    getStreamBufferSizeForTesting(sessionId) {
        const streamState = this.streamState.get(sessionId);
...
function createRetryPlugin(config = {}) {
    ...
    const proxy = {
        name: plugin.name,
        onRequest: (ctx) => plugin.onRequest(ctx),
        onResponse: (ctx) => plugin.onResponse(ctx),
        onStreamChunk: (chunk, sessionId) => plugin.onStreamChunk(chunk, sessionId),
        onStreamEnd: (sessionId) => plugin.onStreamEnd(sessionId),
    };
    // Attach internal methods for testing/graceful access to state
    // @ts-ignore
    proxy._internal = {
        getRequestBody: (key) => plugin.getRequestBodyForTesting(key),
        getRequestBodyJson: (key) => plugin.getRequestBodyJsonForTesting(key),
        getRequestHeaders: (key) => plugin.getRequestHeadersForTesting(key),
        getRetryCount: (key) => plugin.getRetryCountForTesting(key),
        getModifiedBodyForRetry: (key) => plugin.getModifiedBodyForRetry(key),
        getNvidiaWorkerRetryCount: () => plugin.getNvidiaWorkerRetryCount(),
        getUpstream429Counts: () => plugin.getUpstream429Counts(),
        incrementUpstream429Count: (provider) => plugin.incrementUpstream429Count(provider),
        getStreamError: (sessionId) => plugin.getStreamErrorForTesting(sessionId),
        getAndConsumePendingStreamRetry: (sessionId) => plugin.getAndConsumePendingStreamRetry(sessionId),
        getStreamBuffer: (sessionId) => plugin.getStreamBufferForTesting(sessionId),
        getStreamBufferSize: (sessionId) => plugin.getStreamBufferSizeForTesting(sessionId),
