 ...
 */
function sessionProviderKeyGenerator(ctx) {
...
export class RateLimiterPlugin {
     ...
     */
    getBucket(key, provider, sessionId) {
     ...
     */
    pruneWindow(bucket, windowStart) {
     ...
     */
    calculateRetryAfter(bucket, windowMs, maxRequests) {
     ...
     */
    scheduleQueueTimer(bucket, key, provider) {
     ...
     */
    processQueue(bucket, key, provider) {
        const now = Date.now();
        const pConfig = this.getProviderConfig(provider);
        const windowStart = now - pConfig.windowMs;
 ...
 * ```
 */
export function createRateLimiterPlugin(config = {}) {
    const limiter = new RateLimiterPlugin(config);
    const plugin = {
        name: "rate-limiter",
        onRequest: (ctx) => limiter.onRequest(ctx),
    };
    // Attach internal methods for testing/graceful shutdown
    plugin._internal = {
        getBucketState: (key) => limiter.getBucketState(key),
