...
describe("retry plugin - integration tests", () => {
    ...
    describe("5xx responses with exponential backoff", () => {
        ...
        it("retries on 500 three times with exponential backoff, then succeeds", async () => {
            ...
            assert.ok(totalElapsed >= expectedMinDelay && totalElapsed <= expectedMaxDelay, `Total time ${totalElapsed}ms should be approximately 350ms (50+100+200) for 3 retries`);
    ...
    describe("stream buffer overflow protection", () => {
        ...
        afterEach(async () => {
            upstreamServer?.close();
            const origProxy = global.originalProxy;
            const origPlugin = global.originalRetryPlugin;
            if (origProxy)
                proxy = origProxy;
            if (origPlugin)
        ...
        it("clears streamBufferChunks after flushBufferAndStream", async () => {
            ...
            await new Promise((resolve) => upstreamServer.listen(0, resolve));
            upstreamPort = getServerPort(upstreamServer);
            testProxy.upstreams.anthropic = `http://127.0.0.1:${upstreamPort}`;
            const chunks = [];
            await new Promise((resolve, reject) => {
                const req = http.request({
                    hostname: "127.0.0.1",
...
//# sourceMappingURL=retry-plugin.integration.test.js.map