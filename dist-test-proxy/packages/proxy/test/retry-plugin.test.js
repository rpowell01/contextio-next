...
describe("retry plugin - unit tests", () => {
    ...
    describe("onRequest - request buffering", () => {
        ...
        it("generates new x-retry-id when none exists", async () => {
            ...
            assert.match(result.headers["x-retry-id"], /^retry-\d+-\d{6}$/);
        });
    });
    ...
    describe("onResponse - 5xx responses with exponential backoff", () => {
        it("retries 500 with exponential backoff", async () => {
            ...
            assert.equal(plugin._internal.getRetryCount(ctx.captureId), 1);
        });
        ...
        it("increases delay exponentially on subsequent retries", async () => {
            ...
            assert.equal(plugin._internal.getRetryCount(ctx.captureId), 3);
        });
    ...
    describe("onResponse - non-retryable status codes", () => {
        it("passes through 400 immediately without retry", async () => {
            ...
            assert.equal(plugin._internal.getRetryCount(ctx.captureId), 0);
        });
    ...
    describe("onStreamEnd - streaming retry signaling", () => {
        ...
        it("does not signal retry for non-rate-limit error without numeric code (authentication_error)", async () => {
            ...
            const errorChunk = Buffer.from('data: {"type":"error","error":{"type":"authentication_error","message":"Invalid API key"}}\n\n');
            plugin.onStreamChunk(errorChunk, "test-session-123");
    ...
    describe("disabled plugin", () => {
        ...
        it("does not retry when maxRetries=0 but plugin is enabled", async () => {
            ...
            assert.equal(zeroRetryPlugin._internal.getRetryCount(ctx.captureId), 0);
        ...
        it("does not retry when retryableStatuses is empty array", async () => {
            ...
            assert.equal(noRetryStatusesPlugin._internal.getRetryCount(ctx.captureId), 0);
...
//# sourceMappingURL=retry-plugin.test.js.map